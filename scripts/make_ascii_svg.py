...
SRC = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "source-prepped.jpeg")
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "nin-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">ninja1029@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'ninja1029@github:~$ whoami <tspan fill="{INK}">Sudharsan M</tspan></text>')
parts.append(f'<rect x="{PAD+240}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
